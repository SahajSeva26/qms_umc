...
*Last updated: 2026-08-17 (context refresh — no code changes; reconciled with committed `feature/inventory-management` state). Since 2026-08-13: **inventory-assignment** (who-holds-what, per-FO) and **inventory-request** (refill/return lifecycle) are now BUILT + wired + manage-guarded; a platform **`inventory-manager`** role type was added on seeding; and **lead creation** now defaults `salesPerson` to `tenant.salesPerson` (override is `lead:manage`-only). Still scaffolded-only: **inventory-transaction** (the stock ledger) — plus wiring the request `received` step to actually move stock. See the 2026-08-14/16 session-log rows and `backend/CLAUDE.md`.)*
*Update this file at the end of every session.*
