...
*Last updated: 2026-08-31 (branch `feature/test`) — built out the **`test`** module (per-screening results + **FO stock deduction**). On test create, in one txn the test is saved and the camp's assigned FO has stock drawn down per `TestMaster.consumption`: each line's catalog `InventoryMaster` is bridged to the FO's `InventoryConsumable` lots and the exact `rate` subtracted across their `InventoryAssignment` holdings (`adjustHolding`); short stock → 409 + rollback; devices (rate 0) skipped. Also: dedup reuses `TestService.search` (before the txn), FO own-scope on get/search, redundant assigned-FO check dropped from `update`. Completes the camp-day flow: camp live → patient → screening → consent OTP → complete → **test (deducts stock)**. e2e 19/19 on live rs0, `tsc` clean, **uncommitted**. See the 2026-08-31 session-log row, `backend/CLAUDE.md`, and [[test-module]]. Prior entry:*
