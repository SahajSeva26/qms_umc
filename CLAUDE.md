...
*Last updated: 2026-08-05 (backend `fixes/division`: (1) tenant admin role code flattened to plain `admin` + RoleService.create dup-check scoped to {tenant, code}; (2) division now REQUIRED for pharma/customer-tenant roles via new `handleDivision()` helper, admin exempt by role-type code, CREATE-only; both e2e-verified, uncommitted — see session log + memory `role-code-conventions` / `todo-role-division-required-customer`).*
*Update this file at the end of every session.*
