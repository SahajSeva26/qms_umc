...
src/modules/
  ...
  operations/               camp, geoProfile (field-staff geo + camp allocation),
                            ...
                            test (per-screening test result + FO stock deduction)
...
| access-management | tenant | `/tenants` | types: `platform` / `customer`; owner auto-activated on create; optional updatable `salesPerson` (Role ref) — assign on create, reassign/unassign (null) on update, validated to exist AND be a `sales-rep` role type; optional embedded `address` (subdoc, own `coordinates` 2dsphere); optional `businessLifetime` (Number) + `gst` (String — **GSTIN format validated in the VALIDATORS only**, model stores plain string) |
...
| crm | doctor | `/doctors` | tenant-scoped registry (`ctx.where()`); `pharmaCode` immutable natural key, unique per tenant `{tenant, pharmaCode}` (email also unique per tenant); tenant pinned on create (platform supplies, customer own-tenant), open reads / manage-guarded writes. **`create` translates E11000 → clean 409** (race-safe). **`POST /doctors/bulk`** — CSV upload (multipart, file field `file` + optional `tenant`), guarded `doctor:manage`, mirrors MR bulk (CsvHelper→per-row schema validate→`processInBatches(create,5)`); **in-file pharmaCode dedup** (first wins) + invalid/failed rows surfaced in `errors[]`. NOTE stale "global" text in swagger + flat location fields (not the embedded `address` pattern) — flagged, not migrated |
| operations | camp | `/camps` | stageHistory + moveStage; tenant+division required, project optional; optional `tenant` search filter honoured only for a not-tenant-pinned `camp:manage` actor. **FO slot-overlap** primitives (`SLOT_OVERLAPS`/`overlappingSlots`/`isFoBookedForSlot`/`assertFoAvailableForSlot`): 3 daytime slots (9am-1pm/10am-2pm/11am-3pm) overlap each other, evening (6pm-10pm) alone; `create`+`update` hard-block (409) a supplied FO already booked (confirmed/live) on an overlapping slot that day; `moveStage` confirm-guard + auto-assign use the same rule. **`POST /camps/booking-availability`** (guard `camp:book`; a customer-tenant service guard exists but is currently commented out): validates `projectID` belongs to the caller (`ProjectService.get` scoped → foreign project 404); reports per-day/per-slot FO availability for the point+range (≤15 days). FO `$geoNear` + camps lookups are intentionally NOT tenant-scoped (FOs are global platform staff; an FO booked by any pharma is unavailable to all). Slots reported = the project's `campTimeSlots` (fallback: all 4). Response `data.dates` is a **map keyed by `YYYY-MM-DD`** → `{available, slots}`. **FO ALLOCATION is GLOBAL across create/book/allocate (2026-09-14):** `POST /camps/book` runs `create()` under the pharma CUSTOMER ctx, so the FO geo lookup (`nearestFoProfilesGlobal`), FO role validation (`RoleModel.findById().populate('type')` + field-officer assert — NOT `RoleService.get(ctx)`), and clash checks (`bookedFoRoleIdsOnDate`/`isFoBookedForSlot`, no `ctx.where()`) are all platform-wide — otherwise a pharma booker saw 0 FOs and the camp saved with `fo:null`. Pharma authorization + camp ownership stay customer-scoped |
| operations | geoProfile | `/geo-profiles` | field-staff geo (2dsphere); `findNearest` ($geoNear) feeds camp allocation. Carries a **flat address** (2026-09-08): `addressLine1`/`addressLine2`/`locality`/`city`/`state`/`country`(default India)/`pincode`/`googlePlaceId`, all optional, spread FLAT (not nested) so NO second 2dsphere index is created — the top-level `coordinates` doubles as the address geo point, keeping `$geoNear` on a single geo index. Controller/routes unchanged (schema-driven) |
...
| operations | patient | `/patients` | **global registry** (no tenant, per decision); `code` auto-generated from `patient` counter (`pat-`), immutable; `email` optional + lowercased + **sparse-unique**; names trimmed (not lowercased); `dateOfBirth`/`gender`/`mobile`; embedded `address` (`_id:false`); `createdBy` (Role); `active`/`inactive` soft-delete (manage-gated). Perms `patient:manage/create/search/get/update`, reads locked. `create` granted to both camp-coordinators; **field-officer granted create/get/search/update** (full CRUD, reads own-scoped at the camp) |
| operations | screening | `/screenings` | **tenant-scoped**, tenant DERIVED from the camp; one screening per (tenant,patient,camp) unique; `stageHistory` + `moveStage` (`pending→completed/cancelled`, seeds a created entry); **`performedBy`** (Role, **required**) pinned at create = the camp's assigned FO (`camp.fo`) — safe-required because create is gated on camp `live` and a camp can't go live without an FO (service still guards with a clean 409 if absent); embedded `consent` (`otp` generated via `OtpHandler`, `verified`, `signature`); `POST /:id/verify-consent` matches OTP; completion gated on `consent.verified` (422); create gated on camp `live` (409). **Create/mutate:** only the assigned FO (`camp.fo`) or a manage actor (`assertAssignedFoOrManage` via `loadCampForAction`). **Read own-scope:** non-`screening:manage` actor is pinned to their own `performedBy` in get/search (`applyOwnScope`, applied up-front in search since widening filters are manage-gated) — so a non-assigned FO 404s on get and mutations too. Optional `performedBy` search filter, **honoured only for a manage actor** (like `tenant`). Reuses `CampService.get` (populated fo) + `ScreeningService.search` (dedup) via shared `loadCampForAction`. Perms `screening:manage/create/search/get/update`; **FO granted create/get/search/update** (reads own-scoped) |
| operations | test | `/tests` | **per-screening test result**; refs `screening`/`type` (TestMaster)/`tenant` (from screening)/`performedBy` (acting role); only `result` (`{key,value,unit,interpretation?}`) editable — rest pinned at create. **Gates:** camp must NOT be terminal — no test once the camp is `closed`/`cancelled`/`cancelled_charged` (409); screening must be `completed` (409); one result per (screening,type) — dedup **reuses `TestService.search`** (runs BEFORE the txn — search fires count+find in parallel, forbidden in a txn), backed by a unique index `{screening,type}`. **Own-scope:** non-manage actor (the FO) sees/edits only tests they `performedBy` (`applyOwnScope`, applied up-front in get/search; `tenant` filter manage-gated) — so `update` needs no extra assigned-FO check (get already 404s others); `create` still authorizes the assigned FO via `loadScreeningForAction` (returns `{screening,camp}`). **FO stock deduction (the point):** on create, in one `withTransaction`, the test is saved and the **camp's assigned FO** (`camp.fo`) has stock drawn down per `TestMaster.consumption` — each line's `item` is a catalog `InventoryMaster`, bridged to the FO's `InventoryConsumable` lots of it, then the exact `rate` subtracted across those `InventoryAssignment` holdings via `adjustHolding` (devices carry rate 0 → skipped). Short stock → 409, whole create rolls back. Perms `test:manage/create/search/get/update`, reads locked; **FO granted create/get/search/update** (reads own-scoped to their `performedBy` tests). e2e-verified (19/19 on live rs0) |
| inventory | inventory-master | `/inventory-masters` | **global/system catalog** (no tenant scoping); `code` immutable natural key; `type` = device/consumable/accessory/other; `active`/`inactive` status is a soft-delete visibility flag (manage-gated); `sku`/`unit`/`minStock` (**`maxStock` removed 2026-09-08** — dropped from all layers; existing docs keep the orphan field, never read/returned). Reads open, writes guarded `inventory-master:manage` |
| inventory | inventory-device | `/inventory-devices` | individual physical unit of a catalog item; refs `item` (InventoryMaster) + **required immutable `vendor` (VendorMaster, the supplier bought from — validated at create, populated, searchable)** + unique `serialNumber` (all immutable); `location` warehouse/field-officer/camp; `status` = operational lifecycle (available/assigned/maintainance/lost/damaged — NOT soft-delete, fully readable); calibration/warranty/mfg dates. No tenant scoping. Reads open, writes guarded `inventory-device:manage` |
| inventory | inventory-consumable | `/inventory-consumables` | physical stock **lot** of a catalog item; lot identity = unique (`item`,`batch`); `item` immutable; **required immutable `vendor` (VendorMaster — validated at create, populated, searchable)**; `quantity`/mfg date; **`expiryDate` now OPTIONAL (2026-09-08)**; `active`/`expired` status (manage-gated visibility); search sorts by `expiryDate` asc (⚠️ null-expiry lots sort FIRST). No tenant scoping. Reads open, writes guarded `inventory-consumable:manage` |
...
| — | vendor-master | `/vendor-masters` | **global + platform-only** vendor/supplier registry (NO tenant scoping, but **every endpoint incl. reads is permission-guarded** so only platform roles reach it — customer actors 403). `code` immutable uppercase natural key; `name`; `contacts[]` = `{name(req),number,email,designation}`; optional embedded `address` (coordinates optional, 2dsphere); `active`/`inactive` soft-delete (manage-gated visibility + status field hidden from non-manage). Perms `vendor-master:manage/create/get/search/update`; **`:manage` granted to `inventory-manager` role type**. Referenced as a required immutable `vendor` on inventory-device + inventory-consumable |
