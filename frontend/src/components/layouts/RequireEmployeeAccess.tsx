...
interface RequireEmployeeAccessProps {
  children: ReactNode
}

// Employee routes are gated by role-TYPE (RoleGuard), not permission codes — RequirePermission
// can't express that, so this mirrors its exact skeleton with a role-type check instead.
const RequireEmployeeAccess = ({ children }: RequireEmployeeAccessProps) => {
  const { session, permissions, isSettled, isFetching, isError, isConfirmedUnauthenticated, refetch } = usePermission()

  // Latches isError because React Query resets it to false the instant a retry starts — gated
  // on !session in both directions since a background refetch can fail while a cached session is still served.
  ...
  if (!canReadEmployees(session?.roleType.code, permissions)) {
