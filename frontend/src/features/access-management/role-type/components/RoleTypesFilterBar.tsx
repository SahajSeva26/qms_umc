...
const STATUS_OPTIONS: { value: RoleTypeStatus; label: string }[] = [
...
interface RoleTypesFilterBarProps {
  filters: RoleTypesFilterState
  setFilter: <K extends keyof RoleTypesFilterState>(key: K, value: RoleTypesFilterState[K]) => void
  reset: () => void
    ...
    >
      <Select value={filters.status} onValueChange={(v) => setFilter('status', (v ?? 'ALL') as RoleTypesFilterState['status'])}>
        <SelectTrigger className="text-[12px]">
          <SelectValue>{(v: string) => (v === 'ALL' ? 'Status' : (STATUS_LABEL_BY_VALUE.get(v as RoleTypeStatus) ?? 'Status'))}</SelectValue>
        </SelectTrigger>
        <SelectContent>
...
export default RoleTypesFilterBar
