...
//
//   <RequirePermission anyOf={[...]}><DedicatedOpsPage /></RequirePermission>
//
...
// this route in RequirePermission with an invented code, or with none at all
// (`anyOf={[]}`), would not add security: hasAnyPermission([]) evaluates
// false for anyone without `system:manage`, so it would silently lock every
// current user out of a feature that works today. Guessing a plausible-
// looking code would be worse — it would look guarded while enforcing
// nothing, since usePermission() only ever holds codes the server actually
// issued.
//
...
// (module + AuthorizeMiddleware guards on the future API), wrap this route
// exactly like projects.routes.tsx does. Until then, this is enforced nowhere
// but the backend endpoints themselves once they exist — frontend RBAC is
// UI/access control, not security.
