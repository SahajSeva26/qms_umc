...
import { FiPlus, FiClock, FiFileText, FiAlertCircle, FiRefreshCw, FiCornerUpLeft } from 'react-icons/fi'
...
const InventoryRequestsPanel = () => {
  ...
  const { report, isLoading: reportLoading, error: reportError } = useInventoryRequestReport(canManage)
  const reportTiles = useMemo<InventoryReportTile[]>(() => {
    if (!report) return []
    const byType = new Map(report.requests.byType.map((t) => [t.type, t.count]))
    return [
      { key: 'total', label: 'Total Requests', value: report.summary.totalRequests, tone: 'brand', icon: FiFileText },
      // pendingRequests is exactly the 'requested'-status count, not requested+approved — labeled "Awaiting approval" to avoid that misread.
      { key: 'awaiting-approval', label: 'Awaiting approval', value: report.summary.pendingRequests, tone: 'amber', icon: FiAlertCircle },
      { key: 'refill', label: INVENTORY_REQUEST_TYPE_LABEL.refill, value: byType.get('refill') ?? 0, tone: 'teal', icon: FiRefreshCw },
      { key: 'return', label: INVENTORY_REQUEST_TYPE_LABEL.return, value: byType.get('return') ?? 0, tone: 'violet', icon: FiCornerUpLeft },
    ]
  }, [report])
