...
// list active Roles of that type. Used only by inventory-assignment's
// create form — inventory-request has no equivalent restriction (the
// requester is always the acting session's own role, never a picked one).
export const useFieldOfficerRoles = () => {
  const { data: foTypeData } = useRoleTypes({ code: 'field-officer', status: 'active' })
  const foTypeId = foTypeData?.data?.items[0]?.id

  const { data: foRoleData, isFetching } = useRoles({ type: foTypeId, status: 'active', limit: '200' }, !!foTypeId)
