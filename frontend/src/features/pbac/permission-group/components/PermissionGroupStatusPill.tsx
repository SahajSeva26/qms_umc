import type { PermissionGroupStatus } from '@/types/pbac.types'

// Mirrors `@/features/pbac/tenant/components/TenantStatusPill.tsx` exactly,
// keyed on PermissionGroupStatus. `status` is only present server-side when
// the caller holds `system:manage` or `tenant:admin` (see
// PermissionGroupEntity['status'] TODO in pbac.types.ts), so this renders a
