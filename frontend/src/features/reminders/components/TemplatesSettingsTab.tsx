...
// engine against sample data rather than hand-rolling a fake context object.
...
// Templates and Config are two independent editors sharing this file only
// for layout; each hydrates its draft once from loaded data, no re-sync effect.
const TemplatesSettingsTab = () => {
  const { templates, isLoading: templatesLoading } = useReminderTemplates()
  const { config, isLoading: configLoading } = useReminderConfig()

  return (
    <div>
      {templatesLoading && (
        <div className="text-[13px] py-6 text-center" style={{ color: 'var(--qms-text-muted)' }}>Loading templates…</div>
      )}
      {!templatesLoading && templates && <TemplatesEditor templates={templates} />}

      {configLoading && (
        <div className="text-[13px] py-6 text-center" style={{ color: 'var(--qms-text-muted)' }}>Loading settings…</div>
      )}
      {!configLoading && config && <ConfigEditor config={config} />}
    </div>
  )
}

const TemplatesEditor = ({ templates }: { templates: ReminderTemplates }) => {
  const { saveTemplates } = useReminderTemplates()

  const [family, setFamily] = useState<TemplateFamily>('wa_fo')
  const [lang, setLang] = useState<ReminderLanguage>('en')
  const [draft, setDraft] = useState<ReminderTemplates>(templates)
    ...
    <div className="grid gap-3 mb-3.5" style={{ gridTemplateColumns: '1.2fr 1fr' }}>
      <div className="rounded-xl border p-3.5" style={PANEL_STYLE}>
        ...
        <div className="flex gap-1.5 mt-2">
          <Button onClick={handleSaveTemplate} disabled={savingTemplate}><FiSave size={14} /> {savingTemplate ? 'Saving…' : 'Save template'}</Button>
...
const ConfigEditor = ({ config }: { config: ReminderConfig }) => {
  const { saveConfig } = useReminderConfig()

  const [cfgDraft, setCfgDraft] = useState<ReminderConfig>(config)
  ...
  return (
    <div className="rounded-xl border p-3.5" style={PANEL_STYLE}>
      ...
      <div className="mt-2.5">
        <Button onClick={handleSaveConfig} disabled={savingConfig}><FiSave size={14} /> {savingConfig ? 'Saving…' : 'Save settings'}</Button>
      </div>
    </div>
  )
}
