...
vi.mock('@/features/tests/test.service', () => ({
  testService: {
    searchTests: vi.fn(async () => ({
      ...
      message: '',
      data: {
        items: [{ id: 't-1', code: 'ecg', name: 'ECG', therapy: 'cardiology', status: 'active', consumption: [{ item: 'd-1', rate: 0 }] }],
        count: 1,
      },
    })),
    getTest: vi.fn(),
...
describe('TestMasterTab', () => {
  ...
  it('shows the New test button and status filter for a test:manage caller', async () => {
    await mockPermissions(['test:manage'])
    await renderTab()
  ...
  it('hides the New test button and status filter for a caller without test:manage', async () => {
  ...
  it('renders no Edit button, and no click target at all, for a caller without test:manage', async () => {
    await mockPermissions([])
    await renderTab()
