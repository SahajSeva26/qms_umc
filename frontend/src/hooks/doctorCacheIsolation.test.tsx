...
// Regression test for the real bug fixed 2026-08-15: useCampsData.ts's mock
// doctor seed list and useDoctors.ts's real backend-wired doctor list used to
// share the literal query key 'doctors', so TanStack's prefix-matching
// invalidateQueries meant every real doctor create/update silently forced an
// unrelated refetch of the unrelated mock list too. This test exercises the
// ACTUAL production hooks (not synthetic stand-ins) to prove the fix holds:
// doctorKeys.all is ['doctors'], useCampsData's mock list is keyed
// ['mockCampDoctors'] — the two must never cross-invalidate.
...
describe('doctors vs mockCampDoctors cache isolation', () => {
  ...
  it('creating a real doctor does NOT refetch useCampsData\'s mock doctor list', async () => {
    const { useCreateDoctor } = await import('@/features/doctors/hooks/useCreateDoctor')
    const { useCampsData } = await import('@/hooks/useCampsData')
    const { doctorsService } = await import('@/features/doctors/doctors.service')

    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const wrapper = makeWrapper(queryClient)

    const { result: campsDataResult } = renderHook(() => useCampsData(), { wrapper })
    await waitFor(() => expect(campsDataResult.current.isLoading).toBe(false))
